...
echo -n $i
grep "Summary" $i/Assembly/NumberOfSequences.txt -A 3|tail -n 1|sed 's/ //g'|awk -F ":" '{printf "\t" $2 "\t"}'
grep "Contigs >= 500" -A 6 $i/Assembly/OutputNumbers.txt |tail -n 6|sed 's/ //g'|awk -F ":" '{printf $2 "\t"}'
grep ">" $i/Assembly/Prodigal/Genes.fa |wc -l|awk -F ":" '{printf $1 "\t"}'
wc -l $i/Assembly/Prodigal/blast/MERGEM-RG.blast.sum.tsv|awk '{printf $1 "\t"}'
wc -l $i/Assembly/Prodigal/blast/MERGEM-IS.blast.sum.tsv|awk '{printf $1 "\t"}'
grep IS $i/Assembly/Prodigal/blast/ResistomeMobilome-Sum.tsv |wc -l|awk '{printf $1 "\t"}'
